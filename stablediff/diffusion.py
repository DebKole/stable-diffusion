...
from stablediff.attention import SelfAttention, CrossAttention
...
class UNET_ResidualBlock(nn.Module): # we are relating the latent with the time embedding so that the output will depend on the combination of both
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x, context):
        # x: (batch_size, features, ht, width)
        ...
        # (batch_size, features, ht, width) -> (batch_size, features, ht * width)
        x = x.view(n, c, h*w)

        # (batch_size, features, ht * width) -> (batch_size, ht * width, features)
        x = x.transpose(-1, -2)
        ...
        #batch_size, ht * width, features -> batch_size, features, ht * width
        x = x.transpose(-1, -2)
